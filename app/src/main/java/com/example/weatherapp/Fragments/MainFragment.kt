...
class MainFragment : Fragment() {
    ...
    private fun init() = with(binding) {
        ...
        vp.adapter = adapter
        TabLayoutMediator(tabLayout4, vp) { tab, pos ->
            tab.text = tLits[pos]
        }.attach()
    }
    ...
    //парсим полученый результат от сервера для списка дней
    ...
    //парсим полученый результат от сервера для карточки
    private fun parseCurrentData(mainObject: JSONObject, weatherItem: WeatherModel){
        ...
        Log.d("MyLog", "City: ${item.maxTemp}, time: ${item.hours}")
    }
